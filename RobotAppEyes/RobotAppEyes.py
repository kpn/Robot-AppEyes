...
def get_search_element(selector):
    try:
        ...
        raise InvalidElementStateException('Please select a valid selector: CSS_SELECTOR, XPATH, ID,'
...
class RobotAppEyes:
    ...
    *Using Selectors*
    ...
    def open_eyes_session(self,
        ...
        eyes.api_key = apikey
        eyes.force_full_page_screenshot = fullPageScreenshot in [True, 'True', 'true', '1', 1]
    ...
    def check_eyes_window(self, name, force_full_page_screenshot=False, includeEyesLog=False, httpDebugLog=False):
        """
        ...
        | Close Eyes Session |  False               |               |                   |       |       |
        """
        if force_full_page_screenshot in [True, 'True', 'true', '1', 1]:
    ...
    def select_eyes_ignore_region_by_selector(self, selector, value, includeEyesLog=False, httpDebugLog=False):
        ...
        target = {}
        for i in value:
            target[i] = Target().ignore(IgnoreRegionBySelector(searchElement, value[i]))
